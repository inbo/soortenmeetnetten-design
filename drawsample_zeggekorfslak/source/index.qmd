...
Na manuele controle van de hokken in 'uitbreiding 2' beluiten we dat deze hokken een zeer lage kans op aanwezigheid van zeggekorfslak hebben.
We weerhouden dus enkele de 'basis' en 'uitbreiding 1' hokken voor de selectie van de steekproef.
...
#   st_write(dsn = "../output/meetnet_zeggekorfslak.gpkg", "steekproefkader", delete_layer = TRUE)

steekproef <- steekproefkader_select %>%
  filter(steekproef) %>%
  arrange(ranking_rel) %>%
  mutate(naam = ifelse(!is.na(locatie_inhaalslag), locatie_inhaalslag, locatie)) %>%
  select(naam, ranking_rel, type_selectie, tot_aantal)

# steekproef %>%
#   st_write(dsn = "../output/meetnet_zeggekorfslak.gpkg", "steekproef", delete_layer = TRUE)
...
We stellen voor om 60 hokken te selecteren die in een meetcyclus van 6 jaar worden geteld (@tbl-overzicht-s).
