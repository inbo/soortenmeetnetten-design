...
populatie_wvl <- read_sf("../data/distributiondata/populaties_poelkikker_west_vlaanderen.shp", crs = 31370) %>%
  select(area_name)
...
```{r}
controle <- read.csv2("../data/REVISIE Jeroen v4_TW.csv")
...
bscols(
  ...
  addCircleMarkers(data = distribution_data_show_sf, color = ~ifelse(to_check, "orange", "blue"), popup = ~label) %>%
...
```{r, eval = TRUE}
st_write(selectie_poelen_steekproefkader_centroid, "../output/steekproefkader_poelkikker.gpkg", "selectie_watervlakken_centroïde", delete_dsn = TRUE)
...
st_write(selectie_poelen_steekproefkader_centroid, "../output/selectie_watervlakken_centroïde.shp", delete_dsn = TRUE)
