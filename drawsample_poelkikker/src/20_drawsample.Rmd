...
grootste_gebied_elementen <- sampleframe %>%
  filter(gebied_id %in% grootste_gebied$gebied_id)
...
sample_elementen <- sampleframe %>%
  ...
  arrange(desc(zoekzone_selectelement), volgorde_zoekzone, volgorde_element) %>%
...
leaflet() %>%
  addTiles() %>%
  addPolygons(data = st_transform(sample_zoekzone, 4326), color = ~(ifelse(selectzoekzone, "yellow", "blue")), label = ~zoekzone) %>%
  addCircleMarkers(data = st_transform(sample_elementen, 4326), color = ~(ifelse(selectelement, "green", "blue")), label = ~polygon_id)
```
