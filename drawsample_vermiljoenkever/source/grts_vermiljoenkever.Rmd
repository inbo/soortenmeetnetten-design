...
Als steekproefeenheid gebruiken we 1 x 1 km hokken volgens het Military Grid Reference System (meestal UTM1-hokken genoemd). 
We maken gebruik van de geopackage `grts_master_utm1_flanders.gpkg`. 
...
```{r}
grts_utm1 <- read_sf("../data/grts_master_utm1_flanders.gpkg", "cell_polygons") %>%
...
Op basis van de aantallen bepalen we per UTM1-hok:
...
aantallen_sf <- aantallen %>%
  ...
  st_join(select(grts_utm1, cellcode))

utm1_aantallen_summary <- aantallen_sf %>%
  st_drop_geometry() %>%
  group_by(cellcode) %>%
...
Het steekproefkader bouwen we op door de informatie over toegankelijkheid en aanwezigheid van Vermiljoenkover aan elk UTM1 hok te koppelen.
...
```{r}
um1_steekproefkader_ruim <- grts_utm1 %>%
  left_join(utm1_anb_summary, by = "cellcode") %>%
  left_join(utm1_enr_summary, by = "cellcode") %>%
  left_join(utm1_aantallen_summary, by = "cellcode") %>%
  mutate(anb_prop = ifelse(is.na(anb_prop), 0, anb_prop),
         enr_prop = ifelse(is.na(enr_prop), 0, enr_prop))

utm1_steekproefkader_specifiek <- um1_steekproefkader_ruim %>%
...
steekproefkader_dissolve <- utm1_steekproefkader_specifiek %>%
  group_by(aanwezig) %>%
...
utm1_steekproefkader_specifiek %>%
  ...
  addPolygons(color = ~show_color, label = ~ranking_rel, group = "utm1 hokken") %>%
  addCircleMarkers(data = aanwezig, label = ~str_c(bron, ": ", status), group = "waarnemingen") %>%
  addLayersControl(
   overlayGroups = c("utm1 hokken", "waarnemingen", "erkende natuurreservaten", "gebieden in beheer van ANB"),
...
```{r}
st_write(utm1_steekproefkader_specifiek, "../output/steekproefkader_vermiljoenkever.gpkg", layer = "utm1_steekproefkader", driver = "gpkg", delete_layer = TRUE)
...
time_range <- tibble(meetnet = "Vermiljoenkever",
                     jaar = c(min(utm1_aantallen_summary$jaar_min):max(utm1_aantallen_summary$jaar_min)))

utm1_aantallen_jaar <- utm1_aantallen_summary %>%
...
utm1_centroid <- read_sf("../data/grts_master_utm1_flanders.gpkg", "cell_center") %>%
  st_transform(crs = 31370)
  
utm1_aantallen_jaar <- utm1_centroid %>%
  inner_join(utm1_aantallen_jaar, by = "cellcode") %>%
  st_transform(crs = 4326)

 ggplot() +
  #geom_sf(data = flanders, alpha = 0, colour = "grey") +
  geom_sf(data = utm1_aantallen_jaar) +
  transition_time(jaar) 
```

```{r}
gganimate(map_vk,  "vermiljoenkever.gif", title_frame = TRUE)
```
