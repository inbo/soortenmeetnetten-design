...
Het bestand `grts_master_mgrs_flanders.gpkg` kan [hier](https://drive.google.com/drive/folders/1hoSdfHeq2pYntenFCujae8RS_q_w91X7) gedownload worden en wordt in de `data` folder van dit project geplaatst.
...
We selecteren de hokken met aanwezigheid Nauwe korfslak en bepalen per hok:

+ aantal observaties
+ totaal aantal waargenomen individuen
+ minimum jaar van waarneming
+ maximum jaar van waarneming
...
+ Polygonen met leefgebied (rode: bevat N2000 of RBB; blauwe: bevat geen N2000 of RBB); klik op polygoon voor samenstelling van leefgebied
...
Eventueel nog andere scenario's met gebied als steekproefeenheid.
...
```{r, eval =FALSE}
st_write(mgrs_steekproefkader_ruim, "../output/steekproefkader_nauwe_korfslak.gpkg", layer = "mgrs_steekproefkader", driver = "gpkg", delete_layer = TRUE, delete_dsn = TRUE)
