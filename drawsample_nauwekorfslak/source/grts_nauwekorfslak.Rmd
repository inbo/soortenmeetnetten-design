...
```{r}
steekproefkader_buffer <- steekproefkader_polygonen %>%
  st_buffer(dist = 2000)
...
gridpunten_show <- steekproefkader_grid %>%
  st_transform(4326) %>%
  mutate(show_color = ifelse(aanwezig, "yellow", "grey"))
...
leefgebied_show %>%
  ...
  addCircleMarkers(data = gridpunten_show, color = ~show_color, group = "250 m x 250 m grid") %>%
  addLayersControl(
...
```{r}
overzicht <- steekproefkader_grid %>%
  st_drop_geometry() %>%
  group_by(aanwezig, leefgebied) %>%
...
steekproefkader_sc4 <- steekproefkader_grid %>%
  mutate(ranking_rel = rank(ranking),
         panel = ceiling(ranking_rel / 10)) %>%
  mutate(steekproef = ranking_rel <= n) %>%
  mutate(show_color = ifelse(aanwezig, 
                             ifelse(steekproef, "yellow", "blue"),
                             ifelse(steekproef, "orange", "grey")))
  
```

Onderstaande tabel toont het resultaat van de steekproeftrekking. 
Van de 60 geselecteerde punten liggen er 40 in polygonen waar de soort nog niet is waargenomen.

Het resultaat is te zien in onderstaande kaart:

+ steekproefpunten in polygonen met aanwezigheid van soort (geel)
+ steekproefpunten in polygonen met afwezigheid van soort (oranje)
+ niet geselecteerde punten in polygonen met aanwezigheid van soort (blauw) 
+ niet geselecteerde punten in polygonen met aanwezigheid van soort (grijs) 

Voor de steekproefpunten tonen we in de kaart ook het geschikt leefgebied binnen het 250 m x 250 m hok. Dit zijn dus de te onderzoeken gebieden.
...
steekproefkader_sc4 %>%
  ...
  group_by(steekproef, aanwezig, kleur) %>%
...
show_hok <- steekproefkader_hok_leefgebied %>%
  filter(ranking %in% show_steekproef$ranking) %>%
  st_transform(4326)
...
show_steekproef %>% 
  ...
  addCircleMarkers(label = ~panel, labelOptions = labelOptions(noHide = TRUE), color = ~show_color, group = "Gridpunten steekproef") %>%
  addCircleMarkers(data = show_rest,  color = ~show_color, group = "Gridpunten niet in steekproef") %>%
  ...
  addLayersControl(
   ...
   overlayGroups = c("Gridpunten steekproef", "Gridpunten niet in steekproef", "Gebieden steekproef", "Aanwezigheid nauwe korfslak", "Gebieden buiten ecoregio Kustduinen"),
    options = layersControlOptions(collapsed = FALSE))
```
...
```{r, eval =FALSE}
st_write(steekproefkader_sc4, "../output/steekproefkader_nauwe_korfslak.gpkg", layer = "steekproefkader_gridpunten", driver = "gpkg", delete_layer = TRUE, delete_dsn = TRUE)

steekproefkader_hok_leefgebied %>%
  st_write("../output/steekproefkader_nauwe_korfslak.gpkg", layer = "steekproefkader_gebieden", driver = "gpkg", delete_layer = TRUE)
...
gebieden_andere %>%
  st_write("../output/steekproefkader_nauwe_korfslak.gpkg", layer = "gebieden_extra", driver = "gpkg", delete_layer = TRUE)
```
